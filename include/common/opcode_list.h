...
X(OP_CONST_INT_IMM,     FORMAT_ABxx,    "| op:8 | A(rd):8   | Bx(imm):16  |    ; rd = imm")
X(OP_CONST_VAL,         FORMAT_AxBx,    "| op:8 | Ax(rd):12 | Bx(val):12  |    ; rd = bool/special float/none")
...
X(OP_PUSH,              FORMAT_Ax,      "| op:8 | ---:12 | Ax(rs):12   |    ; push register")
X(OP_PUSH_INT_IMM,      FORMAT_Axx,     "| op:8 | ---:8  | Axx(imm):16 |    ; push int immediate")
X(OP_PUSH_VAL,          FORMAT_Ax,      "| op:8 | ---:12 | Ax(val):12  |    ; push bool/special float/none")
X(OP_PUSH_CONST,        FORMAT_Axx,     "| op:8 | ---:8  | Axx(idx):16 |    ; push const from CP")

/* call_direct */
X(OP_CALL,              FORMAT_ABC,     "| op:8 | A(ret):8 | B(nargs):8 | C(offset):8 |    ; directly call function")

X(OP_RET,               FORMAT_Ax,      "| op:8 | ---:12 | Ax(rs):12   |    ; return register")
X(OP_RET_INT_IMM,       FORMAT_Axx,     "| op:8 | ---:8  | Axx(imm):16 |    ; return int immediate")
X(OP_RET_VAL,           FORMAT_Ax,      "| op:8 | ---:12 | Ax(val):12  |    ; return bool/special float/none")
X(OP_RET_CONST,         FORMAT_Axx,     "| op:8 | ---:8  | Axx(idx):16 |    ; return const from CP")
