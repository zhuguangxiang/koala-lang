...
Symbol *STable_Add_Var(STable *stbl, char *name, TypeDesc *desc, int konst)
{
  Symbol *sym = STable_Add_Symbol(stbl, name, SYM_VAR, konst);
  if (!sym) return NULL;
...
Symbol *STable_Add_Symbol(STable *stbl, char *name, int kind, int konst)
{
...
static void __symbol_show_fn(HashNode *hnode, void *arg)
{
  ...
  switch (sym->kind) {
    case SYM_VAR: {
      ...
      printf("%s %s ", sym->konst ? "const":"var", sym->name);
