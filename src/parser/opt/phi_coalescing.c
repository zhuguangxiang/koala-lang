    ...
    vector_foreach(cls, &ctx->final_classes) {
        ...
        vector_foreach(member, &cls->members) {
            ...
            for (int i = 0; i < _insn->num_opers; i++) {
                ...
                /* All non-self sources of an eliminated PHI MUST belong to the same congruence
                 * class. This is guaranteed by fixpoint convergence in the analysis phase. If this
                 * assertion fires, the bug is in _try_merge / analysis, NOT in rewrite. Do NOT add
                 * fallback logic here. */
                KlrCongruenceValue *src_repr = _find_repr(ctx, src);
                ASSERT(src_repr == cls->repr);

                /* Same class guaranteed: use raw src directly */
                klr_build_move_before_terminator(pred, lc, src);
        ...
        vector_foreach(member, &cls->members) {
            ...
            /* Replace non-phi or param uses of this eliminated PHI with %lc
             * except move's dst is %lc
             * example
             * %1 = xxxx
             * move %lc, %1
             * move's %1 cannot be replaced by %lc.
             */
            KlrUse *use, *next;
