...
{
  ctx->ctx_stack = malloc(8192000);
  ctx->ctx_stacksize = 8192000;

  ctx->ctx_stack_ptr = (void**)((char*)ctx->ctx_stack + ctx->ctx_stacksize) - 1;
  ctx->ctx_stack_ptr = (void*)((uintptr_t)ctx->ctx_stack_ptr & ~0x0f);//16 byte stack alignment
  --ctx->ctx_stack_ptr;//ctx_stack_pointer must be decremented an even number of times to maintain 16 byte alignement. this decrement is a dummy/filler decrement
  *--ctx->ctx_stack_ptr = para;
  *--ctx->ctx_stack_ptr = NULL; /*dummy return address*/
  *--ctx->ctx_stack_ptr = (void*)entry;
  *--ctx->ctx_stack_ptr = 0;// rbp
  *--ctx->ctx_stack_ptr = 0;// rbx
  *--ctx->ctx_stack_ptr = 0;// r12
  *--ctx->ctx_stack_ptr = 0;// r13
  *--ctx->ctx_stack_ptr = 0;// r14
  *--ctx->ctx_stack_ptr = 0;// r15

  assert(((uintptr_t)ctx->ctx_stack_ptr & 0x0f) == 0);//verify 16 byte alignment
  return 0;
}
...
{
    void*** const from_sp = &fromctx->ctx_stack_ptr;
    void** const to_sp = toctx->ctx_stack_ptr;
