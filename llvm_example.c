...
void build_if(struct builder_if_state *ifthen, LLVMBuilderRef builder, LLVMValueRef condition)
{
...
void builde_endif(struct builder_if_state *ifthen, LLVMBuilderRef builder)
{
  ...
  if (ifthen->false_block) {
    LLVMBuildCondBr(builder, ifthen->condition, ifthen->true_block, ifthen->false_block);
  }
  else {
    LLVMBuildCondBr(builder, ifthen->condition, ifthen->true_block, ifthen->merge_block);
  }
 ...
 */
void create_if_block(LLVMBuilderRef builder, LLVMBasicBlockRef up, LLVMValueRef cond,
  LLVMBasicBlockRef body, LLVMBasicBlockRef else_body)
{
...
void gen_print_hello(LLVMModuleRef mod, LLVMExecutionEngineRef engine)
{
  ...
  LLVMTypeRef str_type = LLVMStructCreateNamed(LLVMGetGlobalContext(), "string");
  ...
  LLVMTypeRef ret_type2 = LLVMFunctionType(LLVMPointerType(str_type, 0), param_types2, 1, 0);
  str_new_func = LLVMAddFunction(mod, "kl_string_new", ret_type2);
  // LLVMSetLinkage(str_new_func, LLVMExternalLinkage);

  LLVMTypeRef arr_type = LLVMStructCreateNamed(LLVMGetGlobalContext(), "array");
...
void main(int argc, char *argv[])
{
  ...
  LLVMTypeRef ret_type2 = LLVMFunctionType(LLVMInt32Type(), param_types, 0, 0);
