...
`Equatable[T]` / `Hashable` / `ToString` 保持**分立**的泛型 trait，普适性在**遵循层**实现：
...
Koala 的运算符重载是**语法钩子，不是 trait 契约**。四条钩子族全部无 trait：
...
- `Arithmetic` / `BitwiseOperators` 这类纯运算符打包 trait 已移除——无消费者、空概念。
...
|-------|------|
| `any` | 空根（自动遵循三契约的载体） |
| `Equatable[T]` / `Comparable[T]` / `Hashable` / `ToString` | 值契约 |
