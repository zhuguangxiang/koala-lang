...
void parser_enter_scope(ParserState *ps, ScopeKind kind, int block)
{
    ...
    debug("Enter scope-%d(%s)", ps->depth, scopestr);
#endif
}
...
static Symbol *find_ident_symbol(ParserState *ps, IdExpr *exp)
{
    ...
    if (sym) {
        debug("find symbol '%s' in scope-%d(%s: %s)", id->str, ps->depth,
    ...
    vector_foreach_reverse(up, &ps->scope_stack, {
        ...
        if (sym) {
            debug("find symbol '%s' in up scope-%d(%s) %s", id->str, depth,
...
static void parse_int(ParserState *ps, Expr *exp)
{
    ...
    debug("parse integer '%ld'", kexp->ival);
    exp->eop = EXPR_OP_RO;
}
...
{
    ConstExpr *kexp = (ConstExpr *)exp;
    debug("parse float '%lf'", kexp->fval);
    exp->eop = EXPR_OP_RO;
}
...
{
    ConstExpr *kexp = (ConstExpr *)exp;
    debug("parse bool '%s'", kexp->bval ? "true" : "false");
    exp->eop = EXPR_OP_RO;
}
...
{
    debug("parse param type '%s'", sym->name);
    ExprType **item;
    vector_foreach(item, bound, {
#if !defined(NLOG)
        BUF(buf);
        desc_to_str((*item)->ty, &buf);
        debug("bound[%d]: '%s'", i, BUF_STR(buf));
        FINI_BUF(buf);
#endif
    });
    debug("end");
