...
void build_fib(KlrModule *m)
{
    ...
    KlrValue *cond =
        klr_build_cmplt(&bldr, val, klr_const_int(2, int64_type_spec(), m), "");
    ...
    val = klr_build_load(&bldr, param, "");
    KlrValue *sub = klr_build_sub(&bldr, val, klr_const_int(1, int64_type_spec(), m), "");
    KlrValue *args1[] = { sub, NULL };
    KlrValue *ret1 = klr_build_call(&bldr, func, args1, 1, "");

    val = klr_build_load(&bldr, param, "");
    sub = klr_build_sub(&bldr, val, klr_const_int(2, int64_type_spec(), m), "");
