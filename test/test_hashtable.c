...
void test_number_hash_table(void)
{
  ...
  table = HashTable_New(number_hash, number_equal);
  assert(table);
  ...
  for (i = 0; i < 1000000; i++) {
    value++;
    number.value = value;
    hnode = HashTable_Find(table, &number);
    struct number *nu = container_of(hnode, struct number, hnode);
    assert(nu);
...
void test_string_hash_table(void)
{
    ...
    int i;
    string_hash = HashTable_New(string_hash_fn, string_equal_fn);
