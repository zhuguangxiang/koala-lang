...
struct _VTable {
    ...
    /* number of functions */
    int num;
    /* function array, terminated with null */
    FuncNode *func[1];
};
...
void pkg_add_type(PkgInfo *pkg, TypeInfo *type);
void pkg_add_var(PkgInfo *pkg, char *name, TypeDesc *desc);
void pkg_add_cfunc(PkgInfo *pkg, char *name, TypeDesc *desc, void *ptr);
void pkg_add_kfunc(PkgInfo *pkg, char *name, TypeDesc *desc, CodeInfo *code);
